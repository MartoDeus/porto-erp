-- Keep operational write access aligned for administrator, supervisor, coordinator and controller roles.
...
grant select, insert, update on table public.bitacora_eventos to authenticated;

grant update on table public.pasajeros_registros to authenticated;
...
comment on policy diesel_kardex_insert_operativos on public.diesel_kardex is
  'Administrator, supervisor, coordinator and controller can add diesel kardex records.';

comment on policy diesel_movimientos_insert_operativos on public.diesel_movimientos is
  'Administrator, supervisor, coordinator and controller can add diesel movements.';
