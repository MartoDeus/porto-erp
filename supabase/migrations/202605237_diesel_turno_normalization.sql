-- Accept UI turn labels like Diurno/Nocturno while storing the FK codes.
...
begin
  ...
  values (
    ...
    v_stock_inicial, v_stock_final, v_total_recarga, v_total_recibido, nullif(payload ->> 'recibido_de', ''),
    v_total_despacho, v_total_transferencia, v_total_consumo,
    v_reingreso, v_diferencia,
    nullif(payload ->> 'capitan', ''), nullif(payload ->> 'motorista', ''), nullif(payload ->> 'acta_sondaje', ''),
    nullif(payload ->> 'vale_recarga', ''), nullif(payload ->> 'vale_despacho', ''),
    coalesce(payload -> 'modulos_estado', '{}'::jsonb), coalesce(payload -> 'cabecera', '{}'::jsonb),
    nullif(payload ->> 'observaciones', ''), v_has_movement,
