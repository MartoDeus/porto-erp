-- El consumo oficial proviene exclusivamente del módulo Consumo.
-- Las actas de sondaje permanecen como evidencia y control independiente.
...
select
  ...
  stock_inicial_dia
    ...
    - consumo_total
    - cantidad_despachada
    - cantidad_transferida as stock_final_dia,
  capitan_dia,
  motorista_dia,
  capitan_noche,
  motorista_noche,
...
comment on function public.recalcular_diesel_kardex(uuid) is
  'Recalcula el stock usando exclusivamente el consumo oficial registrado. Los sondajes son controles independientes.';

comment on view public.v_diesel_resumen_diario is
  'Resumen diario: consumo_total es el consumo oficial; sondaje_neto se conserva solo como dato de control.';
