-- El resumen diario debe respetar la decision del controlador sobre el sondaje.
-- El sondaje_neto se mantiene como dato informativo, pero el stock_final_dia
-- sale del ultimo stock_final guardado en el kardex del dia.
...
consolidado as (
  select
    ...
    (array_agg(stock_final order by turno_orden desc, created_at desc))[1] as stock_final_dia,
...
comment on view public.v_diesel_resumen_diario is
  'Resumen diario: sondaje_neto queda como control; stock_final_dia usa el ultimo stock_final guardado, que respeta cabecera.sondaje_aplica_stock.';
